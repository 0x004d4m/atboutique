            ...
            <div class="row">
                    ...
                    <div class="m-l-25 m-r--38 m-lr-0-xl">
                        <div class="wrap-table-shopping-cart">
                            <table class="table-shopping-cart">
                                <tr class="table_head">
                                    <th class="column-1">{{ __('checkout.product') }}</th>
                                    <th class="column-2"></th>
                                    <th class="column-3">{{ __('checkout.options') }}</th>
                                    <th class="column-4">{{ __('checkout.quantity') }}</th>
                                    <th class="column-5">{{ __('checkout.total') }}</th>
                                </tr>
                                @foreach ($items as $item)
                                    <tr class="table_row">
                                        ...
                                        <td class="column-5">$ $ {{ $item->product->selling_price * $item->quantity }}</td>
                                    </tr>
                                @endforeach
                            </table>
                        </div>
                ...
                <div class="col-sm-10 col-lg-7 col-xl-5 m-lr-auto m-b-50">
                    <div class="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
                        <h4 class="mtext-109 cl2 p-b-30">{{ __('checkout.cart_totals') }}</h4>
                        <div class="flex-w flex-t bor12 p-b-13">
                            <div class="size-208">
                                <span class="stext-110 cl2">{{ __('checkout.subtotal') }}:</span>
                            </div>
                            <div class="size-209">
                        ...
                        <div class="flex-w flex-t bor12 p-t-15 p-b-30">
                            <div class="size-208 w-full-ssm">
                                <span class="stext-110 cl2">{{ __('checkout.shipping') }}:</span>
                            </div>
                            <div class="size-209 p-r-18 p-r-0-sm w-full-ssm">
                                <p class="stext-111 cl6 p-t-2">
                                    {{ __('checkout.shipping_text') }}
                                </p>
                                <div class="p-t-15">
                                    <span class="stext-112 cl8">{{ __('checkout.calculate_shipping') }}</span>
                                    ...
                                    <div class="bor8 bg0 m-b-12">
                                        <input class="stext-111 cl8 plh3 size-111 p-lr-15" type="text" name="address" placeholder="{{ __('checkout.address') }}">
                                    </div>
                                    <div class="bor8 bg0 m-b-22">
                                        <input class="stext-111 cl8 plh3 size-111 p-lr-15" type="text" name="zip_code" placeholder="{{ __('checkout.zip') }}">
                                    </div>
                                    <div class="flex-w">
                                        <div class="flex-c-m stext-101 cl2 size-115 bg8 bor13 hov-btn3 p-lr-15 trans-04 pointer">
                                            {{ __('checkout.update_totals') }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex-w flex-t p-t-27 p-b-33">
                            <div class="size-208">
                                <span class="mtext-101 cl2">{{ __('checkout.total') }}:</span>
                            </div>
...
<script>
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response.error) {
                        ...
                        messageDiv.append('<div class="alert alert-success">{{ __('checkout.coupon_success') }}!</div>');
                    }
                }
            });
        });
        ...
        var allStates = stateSelect.html();

        $('#country-select').change(function() {
            var selectedCountryId = $(this).val();

            stateSelect.html('<option value="">{{ __('checkout.select_state') }}...</option>');
